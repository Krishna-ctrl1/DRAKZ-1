...
const GlobalLoginTransition = () => {
  const { loginTransition, clearLoginTransition } = useAuth();
  if (!loginTransition) return null;
  return (
    <LoginTransition
      role={loginTransition.role}
      userName={loginTransition.name}
      onComplete={clearLoginTransition}
    />
  );
};
