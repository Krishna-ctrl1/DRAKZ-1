...
async function seedLoans() {
  try {
    ...
    console.log(`Seeding loans for ${users.length} users...`);

    // Clear existing loans for all users
    await Loan.collection.deleteMany({});

    for (const user of users) {
      console.log(`  - Seeding loans for: ${user.email}`);
    ...
    console.log("Loans seeded successfully for all users ✅");
  } catch (err) {
