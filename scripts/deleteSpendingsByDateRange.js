...
(async () => {
  try {
    ...
    if (!fromArg || !toArg) {
      console.error('Usage: node scripts/deleteSpendingsByDateRange.js <from YYYY-MM-DD> <to YYYY-MM-DD> [userId]');
      process.exit(1);
    }
    ...
    console.log(`Deleted ${result.deletedCount} spending records between ${start.toISOString()} and ${end.toISOString()}${userIdArg ? ' for user ' + userIdArg : ''}.`);
    process.exit(0);
  } catch (err) {
